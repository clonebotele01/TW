// MapGod v7r28: Multi-first startup with Single fallback and cooldown-free B for full haul.
try {
...
window.MapGodQuickbar.Main = (function () {
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;background:#7d510f;color:white;padding:6px;margin:-10px -10px 10px -10px">
      <b>MG V7R28 multi-cache</b><button id="mgq_x">X</button>
    </div>
  ...
  async function multiScan(forceOrigins, options = {}) {
    try {
      ...
      let origins = originsWithUnits.filter(o =>
        checkTemplateAvailableFor('a', o.units).ok || checkTemplateAvailableFor('b', o.units).ok
      );
      ...
      if (!origins.length) {
        ...
        if (current && (
          checkTemplateAvailableFor('a', current.units).ok ||
          checkTemplateAvailableFor('b', current.units).ok
        )) {
          origins = [current];
      ...
      for (const c of candidates) {
        ...
        for (const templateName of choices) {
          ...
          if (!v.ignoreCooldownB && (existingConflict || localConflict)) continue;
