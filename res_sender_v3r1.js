javascript:(async function () {
    ...
    function renderRows() {
        ...
        $('#rs3-body').html(rows.map(({ village, res }) => `
            <tr data-source-id="${village.id}">
                <td data-label="Source" class="rs3-source"><a href="${escapeHtml(village.url)}">${escapeHtml(village.name)}</a></td>
                <td data-label="Distance">${distance(village.coord, target)}</td>
                <td data-label="Wood" class="rs3-resource"><span class="icon header wood"></span>${format(res.wood)}</td>
                <td data-label="Clay" class="rs3-resource"><span class="icon header stone"></span>${format(res.stone)}</td>
                <td data-label="Iron" class="rs3-resource"><span class="icon header iron"></span>${format(res.iron)}</td>
                <td class="rs3-action"><button class="btn btn-confirm-yes rs3-send" data-source="${village.id}"
    ...
    function mount() {
        $('head').append(`<style id="rs3-style">
            ...
            #rs3-table td:first-child{text-align:left} #rs3-table a{color:#40d0e0!important}
            .rs3-resource{white-space:nowrap}.rs3-resource .icon{vertical-align:-3px;margin-right:3px}
            .rs3-sent{display:flex;align-items:center;gap:5px;flex-wrap:wrap}
            @media(max-width:600px){
                #rs3-root{width:100%;margin:0;padding:6px;overflow:hidden}
                #rs3-controls{display:grid;grid-template-columns:minmax(0,1fr) minmax(105px,.55fr);gap:7px}
                #rs3-controls>*{min-width:0;width:100%}#rs3-village-select,#rs3-coordinate,#rs3-keep{width:100%}
                #rs3-controls label:first-child{grid-column:1/2}#rs3-close{grid-column:1/-1}
                #rs3-target-info{font-size:14px;margin:8px 0}
                #rs3-table-wrap{overflow:visible}#rs3-table{display:block;min-width:0;width:100%}
                #rs3-table thead{display:none}#rs3-table tbody{display:flex;flex-direction:column;gap:8px}
                #rs3-table tbody tr{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:0;
                    border:1px solid #50545c;border-radius:5px;overflow:hidden;background:#32353b!important}
                #rs3-table tbody td{display:flex;justify-content:center;align-items:center;gap:3px;min-width:0;
                    padding:8px 5px;border:0;border-top:1px solid #454950;text-align:center}
                #rs3-table tbody td:before{content:attr(data-label);display:block;color:#bbb;font-size:11px;line-height:1}
                #rs3-table .rs3-source{grid-column:1/-1;justify-content:flex-start;border-top:0;padding:10px;
                    background:#202225;font-weight:bold;overflow-wrap:anywhere}
                #rs3-table .rs3-source:before,.rs3-table .rs3-action:before{display:none}
                #rs3-table .rs3-source a{display:block;max-width:100%;color:#40d0e0!important}
                #rs3-table td[data-label="Distance"]{grid-column:1/-1;justify-content:flex-start;padding:6px 10px}
                #rs3-table .rs3-resource{flex-direction:column;font-size:15px}
                #rs3-table .rs3-action{grid-column:1/-1;padding:6px 8px}
                #rs3-table .rs3-send{width:100%;min-height:40px;font-size:15px}
            }
        </style>`);
