...
(function () {
    ...
    function scheduleSend() {
        ...
        const tick = () => {
            const remaining = targetSendAt.getTime() - getServerNow().getTime();
            ...
            if (remaining <= 30) {
                while (getServerNow().getTime() < targetSendAt.getTime()) {
                    // Busy-wait for the last few milliseconds to reduce browser scheduling lag.
                }
                sendNow();
                return;
            }

            const waitFor = Math.min(100, Math.max(20, remaining - 25));
