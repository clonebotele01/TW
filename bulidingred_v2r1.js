...
window.BuildingRedMulti.Main = (function () {
  ...
  const PENDING_KEY = 'buildingred_v2_pending_reduction_v1';
  ...
  function parseQueue(doc, village, pageUrl, livePage = false) {
    const seen = new Set();
    const candidates = [];
    const selectors = [
      '#buildqueue tr',
    ...
    selectors.forEach(sel => doc.querySelectorAll(sel).forEach(row => {
      ...
      candidates.push({
        ...
        hasControl: !!control,
        actionUrl,
        liveControl: livePage ? control : null,
      });
    }));
  ...
  async function scanAll() {
    ...
    for (let i = 0; i < villages.length; i++) {
      ...
      try {
        if (String(v.id) === String(window.game_data?.village?.id || '')) {
          rows.push(...parseQueue(document, v, location.href, true));
        } else {
  ...
  function savePendingReduction(item) {
    localStorage.setItem(PENDING_KEY, JSON.stringify({
      villageId: String(item.villageId),
      villageCoord: item.villageCoord || '',
  ...
  function clearPendingReduction() {
    localStorage.removeItem(PENDING_KEY);
  }

  function loadPendingReduction() {
    try { return JSON.parse(localStorage.getItem(PENDING_KEY) || 'null'); }
    catch { return null; }
  }

  function clickLiveReduction(item) {
    if (!item?.liveControl) return false;
    item.liveControl.click();
    clearPendingReduction();
    status(`Clicked live 50% reduction for ${item.villageCoord || item.villageName || item.villageId} / ${item.name}.`);
    setTimeout(scanAll, 900);
    return true;
  }

  function finishPendingOnCurrentVillage() {
    const pending = loadPendingReduction();
    if (!pending) return false;
    if (String(pending.villageId) !== String(window.game_data?.village?.id || '')) return false;

    const village = {
      id: String(window.game_data?.village?.id || ''),
      name: window.game_data?.village?.name || pending.villageName || '',
      coord: window.game_data?.village?.coord || pending.villageCoord || '',
      points: 0,
    };
    const liveRows = parseQueue(document, village, location.href, true);
    const threshold = thresholdSeconds();
    const item = liveRows
      .filter(r => r.remaining != null && r.remaining > threshold && r.liveControl)
      .sort((a, b) => b.remaining - a.remaining)[0];

    if (!item) {
      clearPendingReduction();
      status('Pending reduction found for this village, but no eligible live 50% control is visible.');
      return false;
    }

    const forceSpend = document.getElementById('br2_force')?.checked !== false;
    if (!forceSpend) {
      const ok = confirm(`Finish pending 50% premium reduction for:\n\n${item.villageCoord || item.villageName || item.villageId}\n${item.name}\nRemaining: ${formatDuration(item.remaining)}\n\nThis may spend Premium Points.`);
      if (!ok) return false;
    }
    return clickLiveReduction(item);
  }
  ...
  function quickReduce() {
    ...
    if (clickLiveReduction(item)) return;
    ...
    savePendingReduction(item);
    status(`This 50% control is page-bound. Opening ${item.villageCoord || item.villageName || item.villageId}; run BuildingRed v2 there to finish the pending spend.`);
  ...
  function init() {
    ...
    saveSettings();
    render();
    setTimeout(finishPendingOnCurrentVillage, 250);
  }
