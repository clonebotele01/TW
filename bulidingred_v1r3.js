...
window.BuildingRed.Main = (function () {
  ...
  function saveSettings() {
    localStorage.setItem(SETTINGS_KEY, JSON.stringify({
      ...
      forceSpend: document.getElementById('br_force')?.checked !== false,
    }));
  }
  ...
  function quickReduce() {
    ...
    const forceSpend = document.getElementById('br_force')?.checked !== false;
    if (!forceSpend) {
  ...
  function init() {
    ...
    panel.innerHTML = `
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
        ...
        <label><input id="br_force" type="checkbox" ${s.forceSpend === false ? '' : 'checked'}> Force spend PP</label>
    ...
    ['br_min_hours','br_min_minutes','br_force'].forEach(id => {
