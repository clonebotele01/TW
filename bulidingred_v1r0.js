...
window.BuildingRed.Main = (function () {
  ...
  function rowLabel(row, index) {
    const img = row.querySelector('img[title], img[alt]');
  ...
  function findReductionControl(row) {
    return [...row.querySelectorAll('a, button')].find(isReductionControl) || null;
  }
  ...
  function eligibleRows() {
    const threshold = Math.max(0, (+document.getElementById('br_min_minutes')?.value || 0) * 60);
  ...
  function render() {
    ...
    status(eligible.length
      ...
      : `No visible queue item is over ${thresholdMin} minute(s) with a detected 50% reduction control.`);
  }
  ...
  function init() {
    ...
    panel.innerHTML = `
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
        <label>Min minutes <input id="br_min_minutes" type="number" value="${s.minMinutes ?? 60}" min="0" style="width:70px"></label>
    ...
    ['br_min_minutes','br_confirm'].forEach(id => {
