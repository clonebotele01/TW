...
window.MapGodQuickbar.Main = (function () {
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;background:#7d510f;color:white;padding:6px;margin:-10px -10px 10px -10px">
      <b>MG6.3.6 arrival cooldown</b><button id="mgq_x">X</button>
    </div>
  ...
  function markArrival(target, templateName, originId, arrivalAt) {
    if (!arrivalAt) return;
    const all = loadArrivals();
    const list = activeArrivals(target.id, +document.getElementById('mgq_cd').value || 0);
    list.push({ arrivalAt, sentAt: now(), originId: String(originId), originCoord: document.getElementById('mgq_origin').value, template: templateName });
    all[target.id] = list;
    saveArrivals(all);
  }
  ...
  function faPaginationLinks(root, baseUrl) {
    ...
    root.querySelectorAll('a[href]').forEach(a => {
      try {
        ...
        const hasPageParam = [...url.searchParams.keys()].some(key => /(page|from|offset)/i.test(key));
        if (url.origin !== location.origin || screen !== 'am_farm' || !hasPageParam) return;
        url.hash = '';
        links.add(url.href);
      } catch {}
    });
  ...
  function sendFarm(targetId, templateName) {
    ...
    TribalWars.post(
      ...
      function (r) {
        busy = false;
        markSent(target.id);
        markArrival(target, templateName, originId, arrivalAt);
        removeRow(target.id);
  ...
  async function scan() {
    try {
      ...
      document.getElementById('mgq_results').innerHTML = '<table style="width:100%;border-collapse:collapse;background:#fff8e8"><tr style="background:#d2b06d"><th>#</th><th>D</th><th>Coord</th><th>T</th><th>Pts</th><th>CD / ETA</th><th>Go</th></tr>' + last.map((v, i) => '<tr style="background:' + (v.bonus ? '#e8f4ff' : '#fff8e8') + '"><td>' + (i + 1) + '</td><td>' + v.d.toFixed(2) + '</td><td><a target="_blank" href="' + location.origin + '/game.php?screen=info_village&id=' + v.id + '">' + v.coord + '</a></td><td>' + (v.bonus ? 'Bonus' : 'Barb') + '</td><td>' + v.points + '</td><td>' + (v.sentAt ? minutesAgo(v.sentAt) + 'm ' : '') + (v.result ? v.result + ' ' : '') + (v.haulStatus ? v.haulStatus + ' ' : '') + 'A@' + arrivalLabel(v.aArrivalAt) + (v.aConflict ? ' BLOCK' : '') + ' B@' + arrivalLabel(v.bArrivalAt) + (v.bConflict ? ' BLOCK' : '') + '</td><td><button data-mg-a="' + v.id + '" ' + (!aAvail || v.lossBlockA || v.aConflict ? 'disabled title="' + (!aAvail ? 'A unavailable' : v.lossBlockA ? 'A blocked: last report losses/defeat' : 'A arrival cooldown overlap') + '"' : '') + '>A</button><button data-mg-b="' + v.id + '" ' + (!bAvail || v.bConflict ? 'disabled title="' + (!bAvail ? 'B unavailable' : 'B arrival cooldown overlap') + '"' : '') + '>B</button></td></tr>').join('') + '</table>';
  ...
  document.getElementById('mgq_clear').onclick = () => { localStorage.removeItem(HISTORY_KEY); localStorage.removeItem(ARRIVAL_KEY); status('Cleared history and arrival plan. Scan.'); };
