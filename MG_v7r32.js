// MapGod v7r32: name-based origin group allowlist, compact history, and isolated troop budgeting.
try {
...
window.MapGodQuickbar.Main = (function () {
  ...
  const ORIGIN_CACHE_KEY = 'mapgod_origin_cache_v7r30';
  const REPORT_SYNC_KEY = 'mapgod_report_sync_v1';
  const GROUP_MEMBERSHIP_SESSION_KEY = 'mapgod_group_membership_v1';
  const GROUP_MEMBERSHIP_TTL_MS = 5 * 60 * 1000;
  ...
  const REPORT_SYNC_EXPANSION_PAGES = 20;
  const REPORT_SYNC_COVERAGE_VERSION = 1;
  ...
  const allowedGroupIds = new Set((settings.allowGroupIds || []).map(String).filter(Boolean));
  const blockedGroupIds = new Set((settings.blockGroupIds || []).map(String).filter(Boolean));
  const groupNames = new Map(Object.entries(settings.groupNames || {}).map(([id, name]) => [String(id), String(name)]));
  let groupCatalogLoaded = false;
  let groupMembershipCache = (() => {
    try { return JSON.parse(sessionStorage.getItem(GROUP_MEMBERSHIP_SESSION_KEY) || '{}'); }
    catch { return {}; }
  })();
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;background:#7d510f;color:white;padding:6px;margin:-10px -10px 10px -10px">
      <b>MG V7R32 multi-cache</b><button id="mgq_x">X</button>
    </div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
      ...
      <input id="mgq_group" type="hidden" value="0">
      <button id="mgq_groups">Groups</button><span id="mgq_group_summary"></span>
    ...
    <div id="mgq_group_editor" style="display:none;margin:6px 0;padding:6px;border:1px solid #c9a45c;background:#fff8e8">
      <div style="display:flex;gap:5px;flex-wrap:wrap;align-items:center">
        <select id="mgq_group_pick" style="max-width:260px"><option value="">Loading groups...</option></select>
        <button id="mgq_group_allow">Add allow</button>
        <button id="mgq_group_block">Add block</button>
        <button id="mgq_group_unassign">Unassign</button>
        <button id="mgq_group_all">Allow all</button>
      </div>
      <div id="mgq_group_rules" style="margin-top:5px;white-space:pre-wrap"></div>
    </div>
  ...
  cleanupReloadableMapGodCaches(false);
  ...
  const strictVillageCoord = s => { const m = String(s).match(/(?:^|\D)(\d{1,3})\s*\|\s*(\d{1,3})(?:\D|$)/); return m ? { x: +m[1], y: +m[2] } : null; };
  function villageCoordFromElement(element) {
    ...
    addElementSources(element.querySelector?.('.quickedit-label'));
    addElementSources(element.querySelector?.('.quickedit-vn'));
    addElementSources(element.querySelector?.(
      'a[href*="screen=info_village"][href*="id="], a[href*="village="]'
    ));
    // In desktop overview tables, the village identity is in the leading cells;
    // troop cells later in the row must never be interpreted as "211|2" etc.
    [...(element.children || [])].slice(0, 3).forEach(addElementSources);
    for (const source of sources) {
      const parsed = strictVillageCoord(source);
  ...
  function historyTargetId(key) {
    const parts = String(key || '').split(':');
    return parts[parts.length - 1] || '';
  }
  function historySources(entry) {
    const hi = histInfo(entry);
    return [...new Set([...(Array.isArray(hi.sources) ? hi.sources : []), hi.src].filter(Boolean))];
  }
  function slimHistoryEntry(entry) {
    const hi = histInfo(entry);
    const out = {};
    if (hi.coord) out.coord = hi.coord;
    if (hi.dot) out.dot = hi.dot;
    else if (hi.result) out.result = hi.result;
    if (hi.full || hi.loot) out.full = true;
    if (hi.lootKnown === false) out.lootKnown = false;
    if (hi.partial) out.partial = hi.partial;
    if (hi.haul) out.haul = hi.haul;
    if (hi.rem) out.rem = hi.rem;
    if (hi.reportId) out.reportId = +hi.reportId;
    if (hi.reportAt) out.reportAt = +hi.reportAt;
    else if (hi.seen) out.seen = +hi.seen;
    else if (hi.t) out.t = +hi.t;
    if (hi.reason) out.reason = String(hi.reason).slice(0, 160);
    const sources = historySources(hi);
    if (sources.length > 1) out.sources = sources.slice(-3);
    if (sources.length) out.src = sources[sources.length - 1];
    return out;
  }
  function compactHistory(history, globalLimit, detailLimit) {
    const currentOriginId = String(window.game_data?.village?.id || document.getElementById('mgq_origin_id')?.value || '');
    const latestByTarget = new Map();
    const faDetails = [];
    let inputEntries = 0;
    Object.entries(history || {}).forEach(([key, value]) => {
      const hi = histInfo(value);
      if (!hasHistoryStatus(hi) || !hi.coord) return;
      inputEntries++;
      const targetId = historyTargetId(key);
      if (!targetId) return;
      const previous = latestByTarget.get(targetId);
      const proof = compareHistoryProof(hi, previous?.entry);
      if (!previous || proof > 0 || (proof === 0 && histTime(hi) > previous.time)) {
        latestByTarget.set(targetId, { entry: hi, time: histTime(hi) });
      }
      const sources = historySources(hi);
      if (String(key).includes(':') && sources.includes('fa')) {
        const originId = String(key).split(':')[0];
        faDetails.push({
          key,
          entry: hi,
          time: histTime(hi),
          current: originId === currentOriginId ? 1 : 0,
        });
      }
    });
    ...
    [...latestByTarget.entries()]
      ...
      .slice(0, globalLimit)
      .forEach(([targetId, item]) => { compacted[targetId] = slimHistoryEntry(item.entry); });
    faDetails
      .sort((a, b) => b.current - a.current || b.time - a.time)
      .slice(0, detailLimit)
      .forEach(item => { compacted[item.key] = slimHistoryEntry(item.entry); });
    return { compacted, inputEntries, outputEntries: Object.keys(compacted).length };
  }
  function replaceHistoryObject(target, source) {
    Object.keys(target || {}).forEach(key => delete target[key]);
    Object.assign(target, source);
  }
  function cleanupReloadableMapGodCaches(includeCurrent) {
    const keys = [];
    for (let i = 0; i < localStorage.length; i++) keys.push(localStorage.key(i));
    keys.filter(Boolean).forEach(key => {
      const originCacheKey = key.startsWith('mapgod_origin_cache_');
      const targetCacheKey = key.startsWith('mapgod_target_cache_');
      if (!originCacheKey && !targetCacheKey) return;
      if (!includeCurrent && (key === ORIGIN_CACHE_KEY || key === TARGET_CACHE_KEY)) return;
      localStorage.removeItem(key);
    });
  }
  function saveHistory(h) {
    // Fast path after an earlier compaction: avoid re-sorting the full history on
    // every imported FA page. If it no longer fits, the tiered compactor below runs.
    try {
      const json = JSON.stringify(h || {});
      localStorage.setItem(HISTORY_KEY, json);
      window.MapGodQuickbar.lastHistoryStorage = {
        tier: 'direct',
        inputEntries: Object.keys(h || {}).length,
        outputEntries: Object.keys(h || {}).length,
        bytes: json.length,
      };
      return true;
    } catch {}
    const tiers = [
      { global: 8000, detail: 5000, label: 'normal' },
      { global: 5000, detail: 2000, label: 'compact' },
      { global: 2500, detail: 500, label: 'emergency' },
      { global: 1000, detail: 100, label: 'minimal' },
    ];
    let lastError = null;
    for (let i = 0; i < tiers.length; i++) {
      const tier = tiers[i];
      const result = compactHistory(h, tier.global, tier.detail);
      const json = JSON.stringify(result.compacted);
      try {
        localStorage.setItem(HISTORY_KEY, json);
        replaceHistoryObject(h, result.compacted);
        window.MapGodQuickbar.lastHistoryStorage = {
          tier: tier.label,
          inputEntries: result.inputEntries,
          outputEntries: result.outputEntries,
          bytes: json.length,
        };
        if (i > 0 || result.inputEntries !== result.outputEntries) {
          console.log('[MapGodQuickbar] history compacted', window.MapGodQuickbar.lastHistoryStorage);
        }
        return true;
      } catch (e) {
        lastError = e;
        if (i === 0) {
          // These caches are fully reloadable and may occupy most of the shared
          // per-origin quota. Never delete farming history or unrelated keys.
          cleanupReloadableMapGodCaches(true);
          targetCache = null;
          originCache = null;
          try {
            localStorage.setItem(HISTORY_KEY, json);
    ...
    throw lastError || Error('Unable to store compact farming history');
  }
  function loadArrivals() { try { return JSON.parse(localStorage.getItem(ARRIVAL_KEY) || '{}'); } catch { return {}; } }
  function saveArrivals(h) {
    try { localStorage.setItem(ARRIVAL_KEY, JSON.stringify(h)); }
    catch (e) {
      localStorage.removeItem(TARGET_CACHE_KEY);
      targetCache = null;
      localStorage.setItem(ARRIVAL_KEY, JSON.stringify(h));
    }
  }
  ...
  function saveSettings() {
    localStorage.setItem(SETTINGS_KEY, JSON.stringify({
      ...
      allowGroupIds: [...allowedGroupIds],
      blockGroupIds: [...blockedGroupIds],
  ...
  function markSent(targetId, originId) {
    // Successful sends are tracked by ARRIVAL_KEY. Writing a second history entry
    // for every origin-target pair wastes quota without adding farming-result data.
    return { targetId, originId };
  }
  ...
  function parseDedicatedUnitsTable(root, out) {
    ...
    rows.forEach(row => {
      ...
      const c = villageCoordFromElement(row);
      if (!c) return;
  ...
  function parseOriginsFromOverview(root, out) {
    ...
    $desktopRows.each((_, el) => {
      ...
      const c = villageCoordFromElement(el);
  ...
  function groupRulesConfigured() {
    return allowedGroupIds.size > 0 || blockedGroupIds.size > 0;
  }

  function groupDisplayName(id) {
    return groupNames.get(String(id)) || `Group #${id}`;
  }

  function groupRuleLabel() {
    if (!groupRulesConfigured()) return 'all groups';
    const allowed = [...allowedGroupIds].map(groupDisplayName).join(', ') || 'none';
    const blocked = [...blockedGroupIds].map(groupDisplayName).join(', ') || 'none explicit';
    return `allow [${allowed}], block [${blocked}], unlisted blocked`;
  }

  function renderGroupManager() {
    const summary = document.getElementById('mgq_group_summary');
    const button = document.getElementById('mgq_groups');
    const rules = document.getElementById('mgq_group_rules');
    const pick = document.getElementById('mgq_group_pick');
    if (summary) summary.textContent = groupRulesConfigured()
      ? `A${allowedGroupIds.size}/B${blockedGroupIds.size}`
      : 'All';
    if (button) button.title = groupRuleLabel();
    if (rules) {
      const allowed = [...allowedGroupIds].map(groupDisplayName).join(', ') || '(none)';
      const blocked = [...blockedGroupIds].map(groupDisplayName).join(', ') || '(none)';
      rules.textContent = groupRulesConfigured()
        ? `Allowed: ${allowed}\nBlocked: ${blocked}\nUnlisted groups: blocked`
        : 'No group rules: all villages may be farming origins.';
    }
    if (pick) {
      const selected = pick.value;
      const ids = [...new Set([...groupNames.keys(), ...allowedGroupIds, ...blockedGroupIds])]
        .sort((a, b) => groupDisplayName(a).localeCompare(groupDisplayName(b)));
      pick.innerHTML = '';
      const placeholder = document.createElement('option');
      placeholder.value = '';
      placeholder.textContent = ids.length ? 'Select group...' : 'No named groups found';
      pick.appendChild(placeholder);
      ids.forEach(id => {
        const option = document.createElement('option');
        option.value = id;
        const policy = blockedGroupIds.has(id) ? ' [BLOCK]' : allowedGroupIds.has(id) ? ' [ALLOW]' : '';
        option.textContent = groupDisplayName(id) + policy;
        pick.appendChild(option);
      });
      if (ids.includes(selected)) pick.value = selected;
    }
  }

  function mergeGroupCatalog(root) {
    let added = 0;
    const add = (rawId, rawName) => {
      const id = String(rawId || '').trim();
      const name = String(rawName || '').replace(/\s+/g, ' ').trim();
      if (!/^\d+$/.test(id) || +id <= 0 || !name) return;
      if (!groupNames.has(id) || groupNames.get(id) !== name) added++;
      groupNames.set(id, name);
    };
    root.querySelectorAll(
      'select[name="group"] option, select[name="group_id"] option, ' +
      'select[name*="group"] option, select[id*="group"] option, #group_select option'
    ).forEach(option => add(option.value, option.textContent));
    root.querySelectorAll('[data-group-id]').forEach(element => {
      add(element.getAttribute('data-group-id'), element.textContent || element.getAttribute('title'));
    });
    root.querySelectorAll('a[href*="screen=overview_villages"][href*="group="]').forEach(link => {
      try {
  ...
  async function loadGroupCatalog(force = false) {
    if (groupCatalogLoaded && !force) return groupNames;
    mergeGroupCatalog(document);
    if (!groupNames.size || force) {
      const url = TribalWars.buildURL
        ? TribalWars.buildURL('GET', 'overview_villages', { mode: 'units', type: 'own_home', group: 0 })
        : `${location.origin}/game.php?screen=overview_villages&mode=units&type=own_home&group=0`;
      const html = await fetch(url, { credentials: 'same-origin' }).then(r => {
        if (!r.ok) throw Error('group catalog HTTP ' + r.status);
        return r.text();
      });
      mergeGroupCatalog(new DOMParser().parseFromString(html, 'text/html'));
    }
  ...
  function saveGroupMembershipCache() {
    try { sessionStorage.setItem(GROUP_MEMBERSHIP_SESSION_KEY, JSON.stringify(groupMembershipCache)); }
    catch {}
  }

  async function loadGroupVillageIds(groupId, force = false) {
    const id = String(groupId);
    const cached = groupMembershipCache[id];
    if (!force && cached?.ids && now() - cached.t < GROUP_MEMBERSHIP_TTL_MS) {
      return new Set(cached.ids.map(String));
    }
    const baseUrl = TribalWars.buildURL
      ? TribalWars.buildURL('GET', 'overview_villages', { mode: 'units', type: 'own_home', group: id })
      : `${location.origin}/game.php?screen=overview_villages&mode=units&type=own_home&group=${encodeURIComponent(id)}`;
    ...
    while (pending.length && pages < ORIGIN_IMPORT_MAX_PAGES) {
      ...
      seen.add(url);
      pages++;
      status(`Loading group ${groupDisplayName(id)}, page ${pages}...`);
      if (pages > 1) await new Promise(resolve => setTimeout(resolve, ORIGIN_IMPORT_GAP_MS));
      const html = await fetch(url, { credentials: 'same-origin' }).then(r => {
        if (!r.ok) throw Error(`group ${groupDisplayName(id)} HTTP ${r.status}`);
        return r.text();
      });
      const doc = new DOMParser().parseFromString(html, 'text/html');
      mergeGroupCatalog(doc);
      const found = {};
      parseOriginsFromOverview(doc, found);
      Object.values(found).forEach(v => { if (v.id) ids.add(String(v.id)); });
      overviewPageLinks(doc, url, id, 'units').forEach(link => {
        if (!seen.has(link) && !pending.includes(link)) pending.push(link);
      });
    }
    groupMembershipCache[id] = { t: now(), ids: [...ids] };
    saveGroupMembershipCache();
    return ids;
  }

  async function allowedVillageIdsForRules(force = false) {
    if (!groupRulesConfigured()) return null;
    const allowed = new Set();
    if (!allowedGroupIds.size) return allowed;
    for (const groupId of allowedGroupIds) {
      const ids = await loadGroupVillageIds(groupId, force);
      ids.forEach(id => allowed.add(String(id)));
    }
    for (const groupId of blockedGroupIds) {
      const ids = await loadGroupVillageIds(groupId, force);
      ids.forEach(id => allowed.delete(String(id)));
    }
    return allowed;
  }
  ...
  async function loadOrigins(force) {
    ...
    const accountVillageCount = n(window.game_data?.player?.villages);
    const cacheVillageCountChanged = group === 0 && accountVillageCount > 0 &&
      cached?.rows?.length && cached.rows.length !== accountVillageCount;
    if (cacheVillageCountChanged) {
      console.log('[MapGodQuickbar] origin cache village count changed', {
        cached: cached.rows.length,
        account: accountVillageCount,
      });
    }
    if (!cacheVillageCountChanged && cached?.group === group && cached?.unavailable && now() - cached.t < ORIGIN_CACHE_TTL_MS) {
      originCache = cached;
      setProgress(100, 'Origin overview unavailable; using current village', true);
      return cached.rows || [];
    }
    if (!cacheVillageCountChanged && cached?.group === group && cached?.rows?.length && cacheHasUnits && now() - cached.t < ORIGIN_CACHE_TTL_MS) {
      originCache = cached;
  ...
  function parseReportOverview(root, history, targetsByCoord, originsByCoord, previousMaxReportId) {
    ...
    links.forEach(link => {
      ...
      const result = dot === 'green' ? 'win' : (dot === 'yellow' || dot === 'red_yellow') ? 'losses' : dot === 'red' ? 'defeat' : dot;
      // Account reports are target-wide evidence. One newest entry per target is
      // sufficient for Multi and avoids multiplying data by every possible origin.
  ...
  async function syncRecentReports(targets, origins) {
    ...
    const currentOriginIds = [...new Set(origins.map(v => String(v.id || '')).filter(Boolean))];
    const previousOriginIds = new Set((syncState.originIds || []).map(String));
    const newOriginIds = currentOriginIds.filter(id => !previousOriginIds.has(id));
    const coverageMigration = syncState.coverageVersion !== REPORT_SYNC_COVERAGE_VERSION;
    const expandedCoverage = !!previousMaxReportId && (coverageMigration || newOriginIds.length > 0);
    const maxPages = !previousMaxReportId
      ? REPORT_SYNC_BOOTSTRAP_PAGES
      : expandedCoverage ? REPORT_SYNC_EXPANSION_PAGES : REPORT_SYNC_MAX_PAGES;
    ...
    while (pending.length && pages < maxPages && (expandedCoverage || !reachedKnown)) {
      const url = pending.shift();
      if (seen.has(url)) continue;
      seen.add(url);
      pages++;
      setProgress(5 + (pages / maxPages) * 20, `Syncing reports ${pages}/${maxPages}...`);
      status(`Syncing account reports page ${pages}/${maxPages}${expandedCoverage ? ' (new-origin backfill)' : ''} ...`);
    ...
    if (reportRows && newestReportId) {
      try {
        localStorage.setItem(REPORT_SYNC_KEY, JSON.stringify({
          ...
          t: now(),
          pages,
          coverageVersion: REPORT_SYNC_COVERAGE_VERSION,
          originIds: [...new Set([...(syncState.originIds || []).map(String), ...currentOriginIds])],
        }));
      } catch {}
    }
    console.log('[MapGodQuickbar] report sync', {
      pages, reportRows, matched, reachedKnown, failed, expandedCoverage,
      newOriginIds: newOriginIds.length,
    });
    return {
      history, pages, reportRows, matched, reachedKnown, failed,
      bootstrap: !previousMaxReportId,
      expandedCoverage,
      newOriginIds: newOriginIds.length,
    };
  }
  ...
  function templateCapacityFor(templateName, units) {
    const tpl = templates[templateName];
    if (!tpl) return 0;
    const req = Object.entries(tpl.units || {}).filter(([, need]) => need > 0);
    if (!req.length) return 0;
  ...
  function rowCanSendTemplate(v, templateName, smart = false) {
    ...
    if ((templateName === 'a' ? v.aConflict : v.bConflict) && !(templateName === 'b' && v.ignoreCooldownB)) return false;
    return true;
  }
  ...
  function quickSendSmart() {
    ...
    status('No smart target across the plan. Check farming policy, planned template and origin troop capacity.');
  }
  ...
  function sendFarm(rowIndex, templateName) {
    ...
    const conflict = templateName === 'b' && target.ignoreCooldownB
      ? null
      : arrivalConflict(target.id, arrivalAt, cooldownMin);
  ...
  function renderPlan(rows, modeLabel, page = 0, resetProgress = true) {
    ...
    document.getElementById('mgq_results').innerHTML = controls + '<table style="width:100%;border-collapse:collapse;background:#fff8e8"><tr style="background:#d2b06d"><th>#</th><th>Origin</th><th>D</th><th>Coord</th><th>Pts</th><th>Last result</th><th>ETA</th><th>Go</th></tr>' + visibleRows.map(({ v, i }) => {
      ...
      const bDisabled = !bAvail || bPlanBlocked || bPolicyBlocked || bOriginBlocked || (v.bConflict && !v.ignoreCooldownB);
      ...
      const bTitle = v.planningBlockedReason || (!bAvail ? 'B unavailable' : bPolicyBlocked ? 'Policy blocks B' : bPlanBlocked ? 'Not planned: troop budget chose ' + planned.toUpperCase() : bOriginBlocked ? 'Origin blocked: insufficient B troops' : v.ignoreCooldownB ? 'Full haul: B ignores cooldown' : 'B arrival cooldown overlap');
  ...
  async function scan(skipAutoFA = false) {
    try {
      ...
      const allowedVillageIds = await allowedVillageIdsForRules(false);
      if (allowedVillageIds && !allowedVillageIds.has(String(origin.id))) {
        throw Error(`Current origin ${origin.coord} is blocked by group policy (${groupRuleLabel()})`);
      }
      ...
      targets.forEach((base, idx) => {
        ...
        if (!v.planningBlockedReason && v.fullHaul && (v.greenReport || v.partialLoss)) {
          v.forcedTemplate = 'b';
          v.ignoreCooldownB = true;
        }
        if (
          v.d <= max &&
          ((v.bonus && INCLUDE_BONUS) || (!v.bonus && INCLUDE_BARB)) &&
          (!hideCd || !v.cooldown || v.partialLoss || v.redReport || v.ignoreCooldownB)
        ) rows.push(v);
      });
  ...
  async function multiScan(forceOrigins, options = {}) {
    try {
      planDiagnostic = '';
      originTemplateBlocks = new Set();
      saveSettings();
      ...
      const currentOriginId = String(window.game_data?.village?.id || document.getElementById('mgq_origin_id')?.value || '');
      const currentOriginCoord = window.game_data?.village?.coord || document.getElementById('mgq_origin')?.value || '';
      const liveCurrentUnits = { ...availableUnits };
      const loadedCurrentOrigin = loadedOrigins.find(o => String(o.id || '') === currentOriginId);
      if (loadedCurrentOrigin && Object.keys(liveCurrentUnits).length) {
        // The visible Loot Assistant row is the freshest authority for the current
        // village. Overview pages can omit a unit column or lag after a send.
        loadedCurrentOrigin.units = { ...(loadedCurrentOrigin.units || {}), ...liveCurrentUnits };
      }
      const knownOwnVillageIds = new Set(loadedOrigins.map(o => String(o.id || '')).filter(Boolean));
      const allOriginsWithUnits = loadedOrigins.filter(o => o.id && o.coord && o.units && Object.keys(o.units).length);
      if (options.requireOverview && !allOriginsWithUnits.length) {
        throw Error('Account-wide origin troop overview is unavailable');
      }
      const allowedVillageIds = await allowedVillageIdsForRules(!!forceOrigins);
      const groupAllowedOrigins = allowedVillageIds
        ? allOriginsWithUnits.filter(o => allowedVillageIds.has(String(o.id)))
        : allOriginsWithUnits;
      const groupBlockedOrigins = Math.max(0, allOriginsWithUnits.length - groupAllowedOrigins.length);
      const originsWithUnits = groupAllowedOrigins;
      // Planning must spend from isolated working copies. Mutating the cached rows
      // here would deplete the whole plan immediately, then a successful send would
      // deduct the persistent cache a second time.
      let origins = originsWithUnits
        .filter(o => templateCapacityFor('a', o.units) > 0 || templateCapacityFor('b', o.units) > 0)
        .map(o => ({ ...o, units: { ...o.units } }));
      let inactiveOrigins = Math.max(0, originsWithUnits.length - origins.length);
      let originMode = originCache?.source || 'overview';
      if (!origins.length) {
        if (groupRulesConfigured()) {
          throw Error(`No origin permitted with usable A/B troops (${groupRuleLabel()})`);
        }
        const current = currentVillageOrigin();
        if (current && (
          templateCapacityFor('a', current.units) > 0 ||
          templateCapacityFor('b', current.units) > 0
        )) {
          origins = [current];
          knownOwnVillageIds.add(String(current.id));
        ...
        } else if (originsWithUnits.length) {
          throw Error(
            `Origin overview found ${originsWithUnits.length} village(s) with unit data, ` +
            `but none currently has enough own-home troops for template A or B.`
          );
        } else {
      ...
      const ownVillageIds = new Set([...knownOwnVillageIds, ...origins.map(o => String(o.id || '')).filter(Boolean)]);
      const candidates = [];
      ...
      origins.forEach((origin, oi) => {
        ...
        forEachNearbyTargetIndex(origin, max, targetIndex, ti => {
          ...
          // village.txt can remain stale for hours after a capture. Never allow a
          // village known by the live origin overview to remain a farm target.
          if (ownVillageIds.has(String(base.id))) return;
      ...
      const originPlanStats = new Map(origins.map(origin => [String(origin.id), {
        id: String(origin.id),
        coord: origin.coord,
        capacityA: templateCapacityFor('a', origin.units),
        capacityB: templateCapacityFor('b', origin.units),
        candidates: 0,
        plannedA: 0,
        plannedB: 0,
        cooldownRejected: 0,
      ...
      for (const c of candidates) {
        ...
        const originStats = originPlanStats.get(String(origin.id));
        if (originStats) originStats.candidates++;
        ...
        if (v.redReport) {
          if (originStats) {
            originStats.policyRejected++;
            if (originStats.policyExamples.length < 3) originStats.policyExamples.push(v.coord);
          }
        ...
        if (v.unknownReport) {
          if (!farmUnknown) {
            if (originStats) originStats.policyRejected++;
            if (originStats && originStats.policyExamples.length < 3) originStats.policyExamples.push(v.coord);
        ...
        if (v.fullHaul && (v.greenReport || v.partialLoss)) {
          v.forcedTemplate = 'b';
          v.ignoreCooldownB = true;
        }
        if (rows.length >= lim) {
          if (originStats) originStats.limitRejected++;
          continue;
        }

        const choices = v.ignoreCooldownB ? ['b'] : (v.partialLoss ? ['b'] : (v.unknownFarm ? ['a'] : ['a', 'b']));
        if (v.partialLoss) v.forcedTemplate = 'b';
        let chosen = null;
        let rejectedByCooldown = false;
        let rejectedByTroops = false;
        ...
        if (!chosen) {
          if (originStats && rejectedByCooldown) {
            originStats.cooldownRejected++;
            if (originStats.cooldownExamples.length < 3) originStats.cooldownExamples.push(v.coord);
          }
          if (originStats && rejectedByTroops) {
            originStats.troopRejected++;
            if (originStats.troopExamples.length < 3) originStats.troopExamples.push(v.coord);
          }
          if (v.partialLoss && !plannedTargetIds.has(targetKey) && !awarenessByTarget.has(targetKey)) {
            ...
            awarenessByTarget.set(targetKey, v);
          } else if (v.ignoreCooldownB && !plannedTargetIds.has(targetKey) && !awarenessByTarget.has(targetKey)) {
            v.planningBlockedReason = 'Full haul requires B, but B troops are unavailable';
            awarenessByTarget.set(targetKey, v);
        ...
        if (originStats) {
          if (chosen.templateName === 'a') originStats.plannedA++;
          else originStats.plannedB++;
        }
      ...
      const currentStats = originPlanStats.get(currentOriginId);
      if (currentStats) {
        const exampleText = (label, values) => values.length ? ` ${label}[${values.join(',')}]` : '';
        planDiagnostic = `Current origin ${currentStats.coord}: capacity A x${currentStats.capacityA}, B x${currentStats.capacityB}; candidates ${currentStats.candidates}; planned A ${currentStats.plannedA}, B ${currentStats.plannedB}; rejected cooldown ${currentStats.cooldownRejected}${exampleText('CD ', currentStats.cooldownExamples)}, troops ${currentStats.troopRejected}${exampleText('troop ', currentStats.troopExamples)}, policy ${currentStats.policyRejected}${exampleText('policy ', currentStats.policyExamples)}, limit ${currentStats.limitRejected}`;
      } else {
        const currentUnits = loadedCurrentOrigin?.units || liveCurrentUnits;
        const groupBlocked = allowedVillageIds && !allowedVillageIds.has(currentOriginId);
        planDiagnostic = groupBlocked
          ? `Current origin ${currentOriginCoord || currentOriginId}: blocked by group policy`
          : `Current origin ${currentOriginCoord || currentOriginId}: not active; concrete capacity A x${templateCapacityFor('a', currentUnits)}, B x${templateCapacityFor('b', currentUnits)}`;
      }
      window.MapGodQuickbar.lastMultiPlan = {
        currentOriginId,
        currentOriginCoord,
        origins: [...originPlanStats.values()],
      };
      setProgress(100, `Plan ready (${rows.length} commands, ${awarenessRows.length} review)`, true);
      renderPlan(last, `Multi ${groupRuleLabel()}: commands ${rows.length}, review ${awarenessRows.length}, unknown excluded ${ignoredUnknownTargets.size}, active origins ${origins.length}, inactive ${inactiveOrigins}, group-blocked ${groupBlockedOrigins} (${originMode}), candidate pool ${candidates.length}/${candidatesInRange}, reports ${reportSync.matched}/${reportSync.reportRows} from ${reportSync.pages} page(s)${reportSync.expandedCoverage ? `, new-origin backfill ${reportSync.newOriginIds}` : ''}, live commands ${commandSync.matched}/${commandSync.rows}`);
      return true;
    } catch (e) {
      console.error(e);
      status('MULTI ERROR: ' + e.message);
      window.MapGodQuickbar.lastMultiError = e;
      return false;
    }
  }
  ...
  async function refreshCaches() {
    try {
      ...
      sessionStorage.removeItem(GROUP_MEMBERSHIP_SESSION_KEY);
  ...
  async function runMultiWithSingleFallback(forceOrigins = false) {
    const multiOk = await multiScan(forceOrigins, { requireOverview: true });
    if (multiOk) return true;
    if (groupRulesConfigured()) {
      status(`Multi unavailable. Single fallback is disabled while group policy is active (${groupRuleLabel()}).`);
      return false;
    }
    status('Account-wide Multi unavailable; falling back to Single scan...');
    await scan(false);
    return false;
  }

  function commitGroupRuleChange(message) {
    saveSettings();
    renderGroupManager();
    last = [];
    document.getElementById('mgq_results').innerHTML = '';
    setPlanProgress(0, 0);
    status(`${message}\nPolicy: ${groupRuleLabel()}\nRun Multi to build a new compliant plan.`);
  }

  renderGroupManager();
  document.getElementById('mgq_groups').onclick = async () => {
    const editor = document.getElementById('mgq_group_editor');
    const opening = editor.style.display === 'none';
    editor.style.display = opening ? 'block' : 'none';
    if (!opening) return;
    try {
      status('Loading named village groups...');
      await loadGroupCatalog(!groupCatalogLoaded);
      status(`Group manager ready. ${groupRuleLabel()}`);
    } catch (e) {
      console.error(e);
      renderGroupManager();
      status('GROUP ERROR: ' + e.message);
    }
  };
  document.getElementById('mgq_group_allow').onclick = () => {
    const id = document.getElementById('mgq_group_pick').value;
    if (!id) return status('Select a named group first.');
    blockedGroupIds.delete(id);
    allowedGroupIds.add(id);
    commitGroupRuleChange(`Allowed ${groupDisplayName(id)}.`);
  };
  document.getElementById('mgq_group_block').onclick = () => {
    const id = document.getElementById('mgq_group_pick').value;
    if (!id) return status('Select a named group first.');
    allowedGroupIds.delete(id);
    blockedGroupIds.add(id);
    commitGroupRuleChange(`Blocked ${groupDisplayName(id)}.`);
  };
  document.getElementById('mgq_group_unassign').onclick = () => {
    const id = document.getElementById('mgq_group_pick').value;
    if (!id) return status('Select a named group first.');
    allowedGroupIds.delete(id);
    blockedGroupIds.delete(id);
    commitGroupRuleChange(`Unassigned ${groupDisplayName(id)}.`);
  };
  document.getElementById('mgq_group_all').onclick = () => {
    allowedGroupIds.clear();
    blockedGroupIds.clear();
    commitGroupRuleChange('Cleared group policy; all villages are allowed.');
  };

  document.getElementById('mgq_scan').onclick = () => scan(false);
  document.getElementById('mgq_multi').onclick = () => runMultiWithSingleFallback(false);
  ...
  document.addEventListener('keydown', e => {
    ...
    await runMultiWithSingleFallback(false);
  };
