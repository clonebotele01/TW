// MapGod v7r23: origin exhaustion recovery, full FA sync, live command arrivals and bounded Multi planning.
try {
...
window.MapGodQuickbar.Main = (function () {
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;background:#7d510f;color:white;padding:6px;margin:-10px -10px 10px -10px">
      <b>MG V7R23 multi-cache</b><button id="mgq_x">X</button>
    </div>
  ...
  async function loadOrigins(force) {
    ...
    originCache = { t: now(), group, rows: Object.values(found), unavailable: !usable.length };
  ...
  function rowCanSendTemplate(v, templateName, smart = false) {
    ...
    if (smart && (!v.greenReport || v.lossBlockA)) return false;
  ...
  function renderPlan(rows, modeLabel, page = 0, resetProgress = true) {
    ...
    document.getElementById('mgq_results').innerHTML = controls + '<table style="width:100%;border-collapse:collapse;background:#fff8e8"><tr style="background:#d2b06d"><th>#</th><th>Origin</th><th>D</th><th>Coord</th><th>Pts</th><th>Last result</th><th>ETA</th><th>Go</th></tr>' + visibleRows.map(({ v, i }) => {
      ...
      const aDisabled = !aAvail || aPlanBlocked || aOriginBlocked || v.lossBlockA || v.aConflict;
      const bDisabled = !bAvail || bPlanBlocked || bOriginBlocked || v.bConflict;
      const aTitle = !aAvail ? 'A unavailable' : aPlanBlocked ? 'Not planned: troop budget chose ' + planned.toUpperCase() : aOriginBlocked ? 'Origin blocked: insufficient A troops' : v.lossBlockA ? 'A blocked: last report losses/defeat' : 'A arrival cooldown overlap';
      const bTitle = !bAvail ? 'B unavailable' : bPlanBlocked ? 'Not planned: troop budget chose ' + planned.toUpperCase() : bOriginBlocked ? 'Origin blocked: insufficient B troops' : 'B arrival cooldown overlap';
      ...
      return '<tr style="background:' + (v.bonus ? '#e8f4ff' : '#fff8e8') + '"><td>' + (i + 1) + '</td><td title="' + escapeAttr(v.origin?.name || '') + '">' + originLink + '</td><td>' + v.d.toFixed(2) + '</td><td><a target="_blank" href="' + location.origin + '/game.php?screen=info_village&id=' + v.id + '">' + v.coord + '</a></td><td>' + v.points + '</td><td' + historyStyle + '>' + historyText + '</td><td>' + etaText + '</td><td><button data-mg-a-index="' + i + '" ' + (aDisabled ? 'disabled title="' + aTitle + '"' : '') + '>A</button><button data-mg-b-index="' + i + '" ' + (bDisabled ? 'disabled title="' + bTitle + '"' : '') + '>B</button></td></tr>';
    }).join('') + '</table>';
  ...
  async function scan(skipAutoFA = false) {
    try {
      ...
      renderPlan(
        last,
        'Single scan found ' + rows.length +
        ' / bonus ' + last.filter(v => v.bonus).length +
