// MapGod v7r16: resilient origin parsing, bounded Multi planning, Rune exclusion and incremental report sync.
try {
...
window.MapGodQuickbar.Main = (function () {
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;background:#7d510f;color:white;padding:6px;margin:-10px -10px 10px -10px">
      <b>MG V7R16 multi-cache</b><button id="mgq_x">X</button>
    </div>
  ...
  function unitObjectFromInputs($row) {
    ...
    const skipped = ['ram', 'catapult', 'knight', 'snob', 'militia'];
    ...
    $row.find('.unit-item').each((index, element) => {
      ...
      if (!unit || skipped.includes(unit)) return;
    ...
    $row.find('[data-unit]').each((index, element) => {
      const unit = unitFromElement(element, index);
      if (!unit || skipped.includes(unit) || Object.prototype.hasOwnProperty.call(units, unit)) return;
  ...
  function parseOriginsFromOverview(root, out) {
    ...
    const $desktopRows = $root.find('#combined_table .row_a, #combined_table .row_b, table.vis tr.row_a, table.vis tr.row_b');
  ...
  async function loadOrigins(force) {
    ...
    const found = {};
    let pages = 0;
    const diagnostics = { desktopRows: 0, mobileCards: 0, unitItems: 0, added: 0 };
    ...
    console.log('[MapGodQuickbar] origin import', { pages, found: originCache.rows.length, usable: usable.length, diagnostics });
  ...
  async function multiScan(forceOrigins) {
    try {
      ...
      if (!origins.length) {
        throw Error(`Origin overview parsed ${loadedOrigins.length} village(s), but no unit columns. Open the browser console and copy the "[MapGodQuickbar] origin import" entry.`);
      }
      const targets = await loadTargets(false);
