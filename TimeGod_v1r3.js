...
(function () {
    ...
    function getTimerGodParams() {
        ...
        var sendMs = Number(params.get("tgSendMs") || 0);

        return {
            arrivalAt: arrivalMs > 0 ? new Date(arrivalMs) : null,
            sendAt: sendMs > 0 ? new Date(sendMs) : null,
    ...
    function buildUi() {
        ...
        panel.innerHTML = `
            ...
            <div style="margin-top:8px;line-height:1.45">
                ...
                <div>Mode: <strong id="${SCRIPT_ID}-mode">${timerParams.sendAt ? "planner send time" : "arrival minus duration"}</strong></div>
    ...
    function armTimer(durationMs) {
        try {
            ...
            if (durationMs === null && !timerParams.sendAt) {
            ...
            targetSendAt = timerParams.sendAt
                ? new Date(timerParams.sendAt.getTime() - offsetMs)
                : new Date(arrivalAt.getTime() - durationMs - offsetMs);
            hasSent = false;
