...
window.MapGodQuickbar.Main = (function () {
  ...
  async function scan() {
    try {
      ...
      document.getElementById('mgq_results').innerHTML = '<table style="width:100%;border-collapse:collapse;background:#fff8e8"><tr style="background:#d2b06d"><th>#</th><th>D</th><th>Coord</th><th>T</th><th>Pts</th><th>CD</th><th>Go</th></tr>' + last.map((v, i) => '<tr style="background:' + (v.bonus ? '#e8f4ff' : '#fff8e8') + '"><td>' + (i + 1) + '</td><td>' + v.d.toFixed(2) + '</td><td><a target="_blank" href="' + location.origin + '/game.php?screen=info_village&id=' + v.id + '">' + v.coord + '</a></td><td>' + (v.bonus ? 'Bonus' : 'Barb') + '</td><td>' + v.points + '</td><td>' + (v.sentAt ? minutesAgo(v.sentAt) + 'm' : '-') + (v.result ? ' '+v.result : '') + (v.haulStatus ? ' '+v.haulStatus : '') + (v.rem && !v.haulStatus ? ' R'+v.rem : '') + '</td><td><button data-mg-a="' + v.id + '" ' + (!aAvail ? 'disabled title="A unavailable"' : '') + '>A</button><button data-mg-b="' + v.id + '" ' + (!bAvail ? 'disabled title="B unavailable"' : '') + '>B</button></td></tr>').join('') + '</table>';
