...
window.MapGodQuickbar.Main = (function () {
  ...
  const SYNC_CONFIG_KEY = 'mapgod_sync_config_v1';
  const DEFAULT_SYNC_URL = 'https://script.google.com/macros/s/AKfycbzP9LawOlER-LDAA4gRhp0zklvDK2apSlPYvxIYTPT5NebvfpcrtOgf4IukvGhT4jcuEw/exec';
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;background:#7d510f;color:white;padding:6px;margin:-10px -10px 10px -10px">
      <b>MG6.3 safe sync</b><button id="mgq_x">X</button>
    </div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
      ...
      <button id="mgq_export_data">Export</button>
      <button id="mgq_import_data">Import</button>
      <button id="mgq_clear">Clear</button>
    </div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-top:6px">
      <label>Sync URL <input id="mgq_sync_url" value="${syncConfig.url || DEFAULT_SYNC_URL}" style="width:min(72vw,360px)"></label>
      <label>Token <input id="mgq_sync_token" type="password" value="${syncConfig.token || ''}" style="width:140px"></label>
      <button id="mgq_sync_save">Save sync</button>
      <button id="mgq_cloud_up">Cloud up</button>
      <button id="mgq_cloud_down">Cloud down</button>
    </div>
  ...
  function saveSyncConfig() {
    const cfg = {
      url: String(document.getElementById('mgq_sync_url')?.value || '').trim(),
      token: String(document.getElementById('mgq_sync_token')?.value || '').trim(),
    };
    localStorage.setItem(SYNC_CONFIG_KEY, JSON.stringify(cfg));
    status('Sync config saved.');
    return cfg;
  }
  ...
  function scheduleRequest(work) {
    const delay = Math.max(0, nextRequestAt - now());
    nextRequestAt = Math.max(nextRequestAt, now()) + MIN_REQUEST_GAP_MS;
  ...
  function throttledFetchText(url, options) {
    return scheduleRequest(() => fetch(url, options).then(r => {
      if (!r.ok) throw Error('HTTP ' + r.status);
      return r.text();
    }));
  }

  function throttledPost(url, data, success, error) {
    scheduleRequest(() => new Promise(resolve => {
      TribalWars.post(
        url,
        null,
        data,
        function (r) { success(r); resolve(r); },
        function (r) { error(r); resolve(r); }
      );
    })).catch(error);
  }
  ...
  async function exportData() {
    ...
    try {
      ...
      status('Export copied. Paste it into Import on the other device.');
    } catch {
      prompt('Copy MapGod data:', text);
    }
  }
  ...
  function buildSyncPayload() {
    return {
      ...
      player: window.game_data?.player?.id || '',
      history: loadHistory(),
  ...
  function applySyncPayload(payload) {
    ...
    if (data.settings && typeof data.settings === 'object') localStorage.setItem(SETTINGS_KEY, JSON.stringify(data.settings));
  }

  function importData() {
    const text = prompt('Paste MapGod exported data:');
    if (!text) return;
    try {
      applySyncPayload(JSON.parse(text));
      status('Imported data. Re-run or Scan to refresh.');
      scan();
    } catch (e) {
  ...
  function cloudUpload() {
    ...
    if (!cfg.url || !cfg.token) return status('Cloud up needs Sync URL and Token.');
    scheduleRequest(() => {
      const iframeName = 'mgq_sync_iframe_' + now();
      const iframe = document.createElement('iframe');
      iframe.name = iframeName;
      iframe.style.display = 'none';
      iframe.onload = () => {
        status('Cloud up submitted.');
      ...
      data.name = 'data';
      data.value = JSON.stringify(buildSyncPayload());
      ...
      form.submit();
      form.remove();
      status('Cloud up sending...');
    }).catch(e => status('Cloud up error: ' + e.message));
  }

  function cloudDownload() {
    const cfg = saveSyncConfig();
    if (!cfg.url || !cfg.token) return status('Cloud down needs Sync URL and Token.');
    const callbackName = 'mgqSyncCallback_' + now();
    scheduleRequest(() => new Promise((resolve, reject) => {
      const script = document.createElement('script');
      const sep = cfg.url.includes('?') ? '&' : '?';
      window[callbackName] = r => {
        try {
          if (!r?.ok) throw Error(r?.error || 'sync failed');
          applySyncPayload(r.payload || {});
          status('Cloud down loaded. Scanning...');
          scan();
          resolve(r);
        } catch (e) {
          reject(e);
      ...
      script.onerror = () => {
        delete window[callbackName];
        script.remove();
        reject(Error('Cloud down network/script error'));
      };
      script.src = cfg.url + sep + 'token=' + encodeURIComponent(cfg.token) + '&callback=' + encodeURIComponent(callbackName) + '&_=' + now();
      document.body.appendChild(script);
      status('Cloud down loading...');
    })).catch(e => status('Cloud down error: ' + e.message));
  }
  ...
  async function scan() {
    try {
      ...
      const txt = await throttledFetchText(location.origin + '/map/village.txt', { credentials: 'same-origin' });
  ...
  document.getElementById('mgq_export_data').onclick = exportData;
  document.getElementById('mgq_import_data').onclick = importData;
  document.getElementById('mgq_sync_save').onclick = saveSyncConfig;
  document.getElementById('mgq_cloud_up').onclick = cloudUpload;
  document.getElementById('mgq_cloud_down').onclick = cloudDownload;
