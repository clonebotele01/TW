...
window.MapGodQuickbar.Main = (function () {
  ...
  function latestTargetReportEntry(hist, targetId) {
    ...
    Object.entries(hist || {}).forEach(([key, value]) => {
      ...
      if (!best || histTime(hi) > histTime(best)) best = hi;
    });
    return best;
  }
  ...
  function importFAHistoryFrom(root, h, originId) {
    ...
    root.querySelectorAll('#plunder_list tr[id^="village_"]').forEach(el => {
      ...
      h[key] = { ...(histInfo(h[key])), t: now(), seen: now(), coord, dot, result, loot, full: loot, src: 'fa', originId: String(originId || '') };
      c++;
    });
    return c;
  }
  ...
  async function multiScan(forceOrigins) {
    try {
      ...
      renderPlan(last, `Multi group ${+document.getElementById('mgq_group').value || 0}: origins ${origins.length}, candidates ${candidates.length}, live origin units`);
    } catch (e) {
      console.error(e);
  ...
  document.getElementById('mgq_multi').onclick = () => multiScan(true);
