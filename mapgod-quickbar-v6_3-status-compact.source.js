...
window.MapGodQuickbar.Main = (function () {
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;background:#7d510f;color:white;padding:6px;margin:-10px -10px 10px -10px">
      <b>MG6.3</b><button id="mgq_x">X</button>
    </div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
      <label>Org <input id="mgq_origin" value="${window.game_data?.village?.coord || '481|412'}" style="width:80px"></label>
      <label>OID <input id="mgq_origin_id" value="${window.game_data?.village?.id || ''}" style="width:70px"></label>
      <label>D <input id="mgq_dist" type="number" value="20" style="width:50px"></label>
      <label>L <input id="mgq_limit" type="number" value="100" style="width:55px"></label>
      <label>CD <input id="mgq_cd" type="number" value="30" style="width:45px"></label>
      <label><input id="mgq_hide_cd" type="checkbox" checked> hide</label>
      <label><input id="mgq_F" type="checkbox" checked> F</label>
      <label><input id="mgq_bon" type="checkbox" checked> bon</label>
      <label><input id="mgq_bar" type="checkbox" checked> bar</label>
      <button id="mgq_scan">S</button>
      <button id="mgq_copy">C</button>
      <button id="mgq_probe">P</button>
      <button id="mgq_import">Imp</button>
      <button id="mgq_clear">Clr</button>
    </div>
    <pre id="mgq_status" style="background:#fff8e8;border:1px solid #c9a45c;padding:6px;white-space:pre-wrap">R</pre>
  ...
  function getUnitCount(unit) {
    ...
    for (const sel of selectors) {
      ...
      if (el) {
        const val = n(el.textContent || el.value || el.getAttribute('data-count'));
  ...
  function probeFA() {
    ...
    const msg = `A ${templates.a?.id || '-'} ${availabilityLabel('a')} | B ${templates.b?.id || '-'} ${availabilityLabel('b')}\n${(!am || typeof link !== 'string') ? 'Open am_farm' : 'FA OK'}`;
  ...
  function sendFarm(targetId, templateName) {
    ...
    if (!target) return status('Target missing. S again.');
    const { link, templates, originId } = probeFA();
    const tpl = templates[templateName];
    const avail = checkTemplateAvailable(templateName);

    if (typeof link !== 'string') return status('No FA URL');
    if (!tpl?.id) return status(`No tpl ${templateName.toUpperCase()}`);
    if (!avail.ok) return status(`No ${templateName.toUpperCase()}: ${avail.reason}`);
    if (!originId) return status('No oid');
    if (busy) return status('Busy');
    if (now() - lastClickAt < 250) return status('Fast');

    const F = document.getElementById('mgq_F')?.checked;
    if (!F) {
  ...
  async function scan() {
    try {
      ...
      const includeBarb = document.getElementById('mgq_bar').checked;
      const includeBonus = document.getElementById('mgq_bon').checked;
      probeFA();
      ...
      for (const line of txt.trim().split('\n')) {
        ...
        v.dot = match ? (hi.dot || '') : '';
        v.resu = match ? (hi.result==='win'||hi.dot==='green'?'WIN':hi.result==='losses'||hi.dot==='yellow'?'LOSS':hi.result==='defeat'||hi.dot==='red'?'DEAD':hi.dot?hi.dot[0]:'') : '';
        v.hst = match ? (hi.full||hi.loot?'FULL':hi.partial?'PART '+(hi.haul||''):hi.haul&&hi.haul!=='?'?hi.haul:'') : '';
        v.loot = match ? !!hi.loot : false;
        v.haul = match ? (hi.haul || '') : '';
        v.rem = match ? (hi.rem || '') : '';
        v.cooldown = v.sentAt && cooldownMin > 0 && (now() - v.sentAt) < cooldownMin * 60000;
        if (v.d <= max && ((v.bon && includeBonus) || (!v.bon && includeBarb)) && (!hideCd || !v.cooldown)) rows.push(v);
      }

      rows.sort((a, b) => a.d - b.d || b.points - a.points);
      last = rows.slice(0, lim);
      status('F ' + rows.length + '/' + last.length + ' / bon ' + last.filter(v => v.bon).length + '\nA:' + availabilityLabel('a') + ' B:' + availabilityLabel('b') + ' E=1');

      const aAvail = checkTemplateAvailable('a').ok;
      const bAvail = checkTemplateAvailable('b').ok;
      document.getElementById('mgq_results').innerHTML = '<table style="width:100%;border-collapse:collapse;background:#fff8e8"><tr style="background:#d2b06d"><th>#</th><th>D</th><th>Coord</th><th>T</th><th>Pts</th><th>CD</th><th>Go</th></tr>' + last.map((v, i) => '<tr style="background:' + (v.bon ? '#e8f4ff' : '#fff8e8') + '"><td>' + (i + 1) + '</td><td>' + v.d.toFixed(2) + '</td><td><a target="_blank" href="' + location.origin + '/game.php?screen=info_village&id=' + v.id + '">' + v.coord + '</a></td><td>' + (v.bon ? 'Bonus' : 'Barb') + '</td><td>' + v.points + '</td><td>' + (v.sentAt ? minutesAgo(v.sentAt) + 'm' : '-') + (v.resu?' '+v.resu:'')+(v.hst?' '+v.hst:'')+(v.rem&&!v.hst?' R'+v.rem:'') + '</td><td><button data-mg-a="' + v.id + '" ' + (!aAvail ? 'disabled title="A unavailable"' : '') + '>A</button><button data-mg-b="' + v.id + '" ' + (!bAvail ? 'disabled title="B unavailable"' : '') + '>B</button></td></tr>').join('') + '</table>';
  ...
  document.getElementById('mgq_clear').onclick = () => { localStorage.removeItem(HISTORY_KEY); status('Clred. S.'); };
  document.getElementById('mgq_copy').onclick = async () => {
    const t = last.map(v => v.coord).join(' ');
    try { await navigator.clipboard.writeText(t); status('Cp ' + last.length + ' coords'); }
    catch { prompt('C:', t); }
  };
