// MapGod v7r31: quota-safe compact history, strict coordinates, and isolated troop budgeting.
try {
...
window.MapGodQuickbar.Main = (function () {
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;background:#7d510f;color:white;padding:6px;margin:-10px -10px 10px -10px">
      <b>MG V7R31 multi-cache</b><button id="mgq_x">X</button>
    </div>
  ...
  async function multiScan(forceOrigins, options = {}) {
    try {
      ...
      const originsWithUnits = loadedOrigins.filter(o => o.id && o.coord && o.units && Object.keys(o.units).length);
      if (options.requireOverview && !originsWithUnits.length) {
      ...
      if (currentStats) {
        ...
        planDiagnostic = `Current origin ${currentOriginCoord || currentOriginId}: not active; concrete capacity A x${templateCapacityFor('a', currentUnits)}, B x${templateCapacityFor('b', currentUnits)}`;
      }
      ...
      renderPlan(last, `Multi group ${+document.getElementById('mgq_group').value || 0}: commands ${rows.length}, review ${awarenessRows.length}, unknown excluded ${ignoredUnknownTargets.size}, active origins ${origins.length}, inactive ${inactiveOrigins} (${originMode}), candidate pool ${candidates.length}/${candidatesInRange}, reports ${reportSync.matched}/${reportSync.reportRows} from ${reportSync.pages} page(s)${reportSync.expandedCoverage ? `, new-origin backfill ${reportSync.newOriginIds}` : ''}, live commands ${commandSync.matched}/${commandSync.rows}`);
      return true;
    } catch (e) {
      console.error(e);
