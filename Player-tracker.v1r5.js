...
window.MapGodRadiusPlayerTracker.Main = (function () {
  ...
  function saveToday(rows, settings, completed) {
    ...
    Object.keys(all).sort().reverse().slice(MAX_SNAPSHOT_DAYS).forEach(oldDate => delete all[oldDate]);
  ...
  function rankingChangeTable(label, rows, beforeKey, currentKey, deltaKey) {
    const body = rows.map((r, i) => `<tr style="background:${i % 2 ? '#fff8e8' : '#f7ebcd'}">
      <td>${i + 1}</td>
      <td><a target="_blank" href="${location.origin}/game.php?screen=info_player&id=${r.id}">${esc(r.name)}</a></td>
      <td title="${esc(r.tribeName)}">${esc(r.tribe || '-')}</td>
      <td>${fmt(r[beforeKey])}</td>
      <td>${fmt(r[currentKey])}</td>
      <td>${fmtDelta(r[deltaKey])}</td>
    </tr>`).join('');
    return `<div style="flex:1;min-width:360px">
      <b>Top 10 ${label} changes</b>
      <table style="width:100%;border-collapse:collapse;background:#fff8e8;margin-top:4px">
        <tr style="background:#d2b06d"><th>#</th><th>Player</th><th>Tribe</th><th>Before</th><th>Now</th><th>Change</th></tr>
        ${body || '<tr><td colspan="6">No comparable values.</td></tr>'}
      </table>
    </div>`;
  }
  ...
  function renderPointSummary() {
    ...
    const rows = (latest.rows || []).flatMap(row => {
      ...
      return [{
        ...
        beforeOdd: old.odd,
        beforeOds: old.ods,
        odDelta: row.od == null || old.od == null ? null : row.od - old.od,
        odaDelta: row.oda == null || old.oda == null ? null : row.oda - old.oda,
        oddDelta: row.odd == null || old.odd == null ? null : row.odd - old.odd,
        odsDelta: row.ods == null || old.ods == null ? null : row.ods - old.ods,
      }];
    });
    ...
    const topRankingChanges = key => [...rows].filter(r => r[key] != null).sort((a, b) => b[key] - a[key] || b.pointDelta - a.pointDelta).slice(0, 10);
    result.innerHTML = `<div style="margin:6px 0"><b>${latestDate}</b> compared with <b>${beforeDate}</b> (${days} day${days === 1 ? '' : 's'})</div>
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px">
        ${rankingChangeTable('OD', topRankingChanges('odDelta'), 'beforeOd', 'od', 'odDelta')}
        ${rankingChangeTable('ODA', topRankingChanges('odaDelta'), 'beforeOda', 'oda', 'odaDelta')}
        ${rankingChangeTable('ODD', topRankingChanges('oddDelta'), 'beforeOdd', 'odd', 'oddDelta')}
        ${rankingChangeTable('ODS', topRankingChanges('odsDelta'), 'beforeOds', 'ods', 'odsDelta')}
      </div>`;
