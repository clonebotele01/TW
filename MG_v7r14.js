// MapGod v7r14: spatial Multi planning, Rune exclusion and incremental report sync.
try {
...
window.MapGodQuickbar.Main = (function () {
  ...
  function nearbyTargetIndexes(origin, max, index) {
    const found = [];
  ...
  async function loadOrigins(force) {
    ...
    if (cached?.group === group && cached?.rows?.length && now() - cached.t < ORIGIN_CACHE_TTL_MS) {
      originCache = cached;
    ...
    while (pending.length && pages < ORIGIN_IMPORT_MAX_PAGES) {
      ...
      seen.add(url);
      pages++;
      setProgress(Math.min(95, 10 + pages * 10), `Loading origin page ${pages}...`);
    ...
    setProgress(100, `Origin cache ready (${originCache.rows.length})`, true);
  ...
  function enrichTarget(base, origin, hist, cooldownMin, options = {}) {
    ...
    const reportHistory = options.latestReport ? (latestTargetReportEntry(hist, v.id) || rawHistory) : rawHistory;
  ...
  function renderPlan(rows, modeLabel) {
    ...
    setPlanProgress(0, rows.length);
    document.getElementById('mgq_results').innerHTML = '<table style="width:100%;border-collapse:collapse;background:#fff8e8"><tr style="background:#d2b06d"><th>#</th><th>Origin</th><th>D</th><th>Coord</th><th>Pts</th><th>CD / ETA</th><th>Go</th></tr>' + rows.map((v, i) => {
  ...
  async function multiScan(forceOrigins) {
    try {
      ...
      origins.forEach((origin, oi) => {
        ...
        nearbyTargetIndexes(origin, max, targetIndex).forEach(ti => {
          const base = targets[ti];
          const d = dist(origin, base);
          if (d <= max) candidates.push({ oi, ti, d });
        });
      });
      setProgress(45, `Sorting ${candidates.length} candidates...`);
      ...
      for (const c of candidates) {
        ...
        const v = enrichTarget(base, rowOrigin, hist, cooldownMin, { latestReport: true });
      ...
      renderPlan(last, `Multi group ${+document.getElementById('mgq_group').value || 0}: origins ${origins.length}, candidates ${candidates.length}, reports ${reportSync.matched}/${reportSync.reportRows} from ${reportSync.pages} page(s), live origin units`);
    } catch (e) {
      console.error(e);
