...
(function () {
    ...
    function buildUi() {
        ...
        var defaultArrival = new Date(now.getTime() + (durationMs || 0) + 60000);
    ...
    function parseArrivalInput() {
        ...
        var timeValue = document.getElementById(SCRIPT_ID + "-time").value.trim();https://github.com/clonebotele01/TW/tree/main
    ...
    function armTimer(durationMs) {
        try {
            ...
            showMessage("Timer armed. Keep this tab open on the confirm page.", false);

            if (tickHandle) clearInterval(tickHandle);
            tickHandle = setInterval(tick, 25);
            tick();
        } catch (error) {
